...
class SlaveInstance
{
   public:
    ...
    virtual void SetupExperiment(FMIBoolean toleranceDefined, FMIReal tolerance, FMIReal tStart,
    ...
    virtual void SetReal(const FMIValueReference vr[], std::size_t nvr, const FMIReal value[]);
    virtual void SetInteger(const FMIValueReference vr[], std::size_t nvr, const FMIInteger value[]);
    virtual void SetBoolean(const FMIValueReference vr[], std::size_t nvr, const FMIBoolean value[]);
    virtual void SetString(const FMIValueReference vr[], std::size_t nvr, const FMIString value[]);

    // Called from fmiGetXxx(). Throw std::logic_error by default.
    virtual void GetReal(const FMIValueReference vr[], std::size_t nvr, FMIReal value[]) const;
    virtual void GetInteger(const FMIValueReference vr[], std::size_t nvr, FMIInteger value[]) const;
    virtual void GetBoolean(const FMIValueReference vr[], std::size_t nvr, FMIBoolean value[]) const;
    virtual void GetString(const FMIValueReference vr[], std::size_t nvr, FMIString value[]) const;

    // Called from fmiDoStep(). Must be implemented in model code.
    virtual bool DoStep(FMIReal currentCommunicationPoint, FMIReal communicationStepSize, FMIBoolean newStep,
                        FMIReal& endOfStep) = 0;
