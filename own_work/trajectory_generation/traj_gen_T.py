...
# Setup
X_init = [10, 10, pi, 0, 0, 0, 0, 0, 0] # POS VEL ACC
...
for k in range(N):
    ...
    if k < N-1:
        ...
        lbw += [-1e-3, -1e-3, -1e-3] # LB jerk
        ubw += [1e-3, 1e-3, 1e-3] # UB jerk
        w0 += [0,0,0]
    ...
    if k < N-1:
        g += [Xk]
        lbg += [-1e-3, -1e-3, -inf, -MAX_VEL, -MAX_VEL, -MAX_ANG_VEL, -MAX_ACC, -MAX_ACC, -MAX_ANG_ACCEL] # POS VEL ACC
        ubg += [inf, inf, inf, MAX_VEL, MAX_VEL, MAX_ANG_VEL, MAX_ACC, MAX_ACC, MAX_ANG_ACCEL]
    else:
        g += [Xk]
        lbg += [-1e-3, -1e-3, -1e-3, -1e-3, -1e-3, -1e-3, -1e-3, -1e-3, -1e-3] # POS VEL ACC
        ubg += [1e-3, 1e-3, 1e-3, 1e-3, 1e-3, 1e-3, 1e-3, 1e-3, 1e-3]
...
x_opt = [X_init]
for k in range(N):
    Fk = F(x0=x_opt[-1], p=u_opt[k*3:(k+1)*3], t=t_opt)
    x_opt += [Fk['xf'].full()]
